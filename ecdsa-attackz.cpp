...
bool try_bruteforce(BIGNUM* r1, BIGNUM* s1, BIGNUM* z1,
    ...
    while (attempts < max_attempts) {
        ...
        bool eq2_ok = (BN_cmp(temp, RHS2) == 0);
        ...
        if (eq2_ok) {
            ...
            bool s1_ok = (BN_cmp(s1_check, s2) == 0);
            bool s2_ok = (BN_cmp(s2_check, s3) == 0);
    ...
    BN_free(r2_s1); BN_free(r1_s2); 
     BN_free(RHS1); BN_free(RHS2);
     BN_free(s1_check); BN_free(s2_check);
...
string z2_hex = "17ea532a30334538c281467befb5fca7e66c6b1e760f5d17d77cb853c65a3c7d";
    ...
    // ============================================
    cout << "\n" << string(70, '=') << endl;
    found = try_direct_formula_with_delta(r1, s1, z1, r2, s2, z2, ,
    ...
    // ============================================
    cout << "\n" << string(70, '=') << endl;
    found = try_bruteforce(r1, s1, z1, r2, s2, z2, ,
    ...
    // ============================================
    cout << "\n" << string(70, '=') << endl;
    found = try_random(r1, s1, z1, r2, s2, z2, ,
                       order, ctx, d, expected_pub, group, 10000000);
    if (found) {
